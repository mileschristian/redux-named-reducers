 ...
 *
 * @param {Object} options an object of the form { moduleName: <optional: module name>, reducer: <reducer>, externalState: <optional: Object list of external state and their default values> }
...
export function createNamedReducer(options) {
	const namedReducer = function(state, action) {
		if (action.type === _UPDATE_STATE_ACTION_TYPE_DONT_USE_) {
			...
			namedReducer.latestLocalState = state
			return state
		} else {
	...
	namedReducer.latestLocalState = null
	...
	Object.keys(options.reducer(undefined, { type: undefined })).forEach(e => {
		...
		namedReducer[e].selector = function() {
			return namedReducer.latestLocalState[e]
		}
...
export function getState(namedReducerState) {
	return namedReducerState.selector(namedReducerState.namedReducer.latestState) //pass the latest state in case the selector is from reselect
}
